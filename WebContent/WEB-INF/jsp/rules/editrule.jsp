...
<script type="text/javascript"
	src="<c:url value='/js/editrule-form.js'/>"></script>
...
<body>
	...
	<div id="page">
		<div id="content">
			<form id="editrule_form" class="width800"
				action="<c:url value='/rules/updaterule'/>" method="post">
				<fieldset>
					<legend>Editar Regra de Estilo</legend>
					<b>ID da Regra: </b>
					<c:out value="${rule.ruleID}" />
					<input name="updatedRule.ruleID" value="${rule.ruleID}"
						style="display: none"> <br />
					<br />
					<div class="single_form_element">
						<label class="label" for="language">Idioma*</label> <br />
						<c:choose>
							<c:when test="${rule.language == 'PORTUGUESE'}">
								<input id="pt" name="updatedRule.language" value="0"
									type="checkbox" checked="true">Português <input id="en"
									name="updatedRule.language" value="1" type="checkbox">Inglês<br />
							</c:when>
							<c:otherwise>
								<input id="pt" name="updatedRule.language" value="0"
									type="checkbox">Português <input id="en"
									name="updatedRule.language" value="1" type="checkbox"
									checked="true">Inglês<br />
							</c:otherwise>
						</c:choose>
					</div>
					...
					<div class="single_form_element">
						<label class="label" for="name">Categoria*</label> <br /> <select
							id="selectCategory" name="updatedRule.category"
							class="input_border width250">
							<c:forEach var="category" items="${ruleCategories}">
								<c:choose>
									<c:when
										test="${rule.category.categoryDescription == category.categoryDescription}">
					...
					<div class="single_form_element">
						<label class="label" for="name">Tipo*</label> <br /> <select
							id="selectRuleType" name="updatedRule.type"
							class="input_border width250">
							<c:forEach var="type" items="${typesOfRules}">
								<c:choose>
									<c:when
										test="${rule.type.typeDescription == type.typeDescription}">
	...
	int getLemmaPosition() {
		return i;
					...
					<div class="single_form_element" style="display: inline">
						...
						<div id="lemmasDiv" class="indent">
							<c:forEach items="${rule.lemmas}" var="lemma">
								<span id="lemma<%=getLemmaPosition()%>"> <label
									class="label">Padrão incorreto</label><br /> <input
									class="input_border width450" type="text" maxlength=200
									name="lemmas[<%= getLemmaPosition()%>].pattern"
									value="${lemma.pair.pattern}" /><br /> <label class="label">Sugestões</label><br />
									<input class="input_border width450" type="text" maxlength=200
									name="lemmas[<%= getLemmaPosition()%>].suggestions"
									value="${lemma.pair.suggestions}" /><br /> <br /></span>
								<%
									i = i + 1;
								%>
							</c:forEach>
					...
					<input id="lemmasSize" value="<%=getLemmaPosition()%>"
						style="display: none">
					<div class="single_form_element" style="display: inline">
						<div class="indent">
	...
	int getMatchingPosition() {
		return j;
					...
					<div class="single_form_element" style="display: inline">
						...
						<div id="exactMatchingsDiv" class="indent">
							<c:forEach items="${rule.exactMatchings}" var="exactMatching">
								<span id="exactMatching<%=getMatchingPosition()%>"> <label
									class="label">Padrão incorreto</label><br /> <input
									class="input_border width450" type="text" maxlength=200
									name="exactMatchings[<%= getMatchingPosition()%>].pattern"
									value="${exactMatching.pair.pattern}" /><br /> <label
									class="label">Sugestões</label><br /> <input
									class="input_border width450" type="text" maxlength=200
									name="exactMatchings[<%= getMatchingPosition()%>].suggestions"
									value="${exactMatching.pair.suggestions}" /><br /> <br /></span>
								<%
									j = j + 1;
								%>
							</c:forEach>
					...
					<input id="matchingsSize" value="<%=getMatchingPosition()%>"
						style="display: none">
					<div class="single_form_element" style="display: inline">
						<div class="indent">
					...
					<div class="single_form_element">
						...
						<textarea id="explanation"
							...
							class="input_border" name="updatedRule.explanation" maxlength=400><c:out
								value="${rule.explanation}" /></textarea>
						<br />
					</div>
					<br /> <b>Referência</b><br />
					<div class="indentation">
						...
						<button id="changeSource" type="button" class="button">Alterar
							Referência</button>
