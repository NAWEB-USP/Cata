...
<body>
	...
	<div id="page">
		<div id="content">
			...
			<p>Existem muitas maneiras de melhorar o estilo de um texto
				acadêmico: evitar repetição de termos, evitar construções
				linguísticas pouco comuns, entre outras.</p>
			<p>Por enquanto, no Sistema CATA, as sugestões de estilo
				referem-se ao uso inadequado de termos e expressões no texto, como
				uso de clichês, traduções incorretas e estrangeirismos.</p>
			<p>Confira abaixo todas as regras cadastradas atualmente no CATA:
			</p>
			<div class="grid">
						...
						<c:forEach items="${rules}" var="rule">
							<tr>
								...
								<td class="center"><c:out
										value="${rule.language.languageDescription}" /></td>
								<td class="center"><c:out
										value="${rule.type.typeDescription}" /></td>
								<td class="center"><c:out
										value="${rule.category.categoryDescription}" /></td>
								<c:choose>
									<c:when test="${!empty rule.lemmas}">
										<c:forEach items="${rule.lemmas}" var="lemma">
											<c:if test="${lemma.pair.defaultPair == true}">
												...
												<td class="center"><c:out
														value="${lemma.pair.suggestions}" /></td>
											</c:if>
										</c:forEach>
									</c:when>
									<c:otherwise>
										<c:forEach items="${rule.exactMatchings}" var="exactMatching">
											<c:if test="${exactMatching.pair.defaultPair == true}">
												...
												<td class="center"><c:out
														value="${exactMatching.pair.suggestions}" /></td>
											</c:if>
										</c:forEach>
									</c:otherwise>
								</c:choose>
								...
								<td class="center"><img
									onclick="showRuleModal(${rule.ruleID});"
									src="<c:url value='/css/images/plus-icon.png'/>">
									<div id="rule${rule.ruleID}" class="popup_block"
										style="display: none">
										<div class="popup_content" style="text-align: left">
											<b>Mais informações sobre a Regra de Estilo</b><br /> <br />
											<b><c:out value="${rule.category.categoryDescription}" />:</b>
											<c:out value="${rule.type.typeDescription}" />
											<br />
											...
											<c:if test="${!empty rule.exactMatchings}">
												...
												<div class="indentation">
													<table class="modal-table">
														...
														<c:forEach items="${rule.exactMatchings}"
															var="exactMatching">
															<tr>
											...
											<br /> Esta regra foi cadastrada por <i><c:out
													value="${rule.user.name}" /></i>
										</div>
									</div></td>
							</tr>
						</c:forEach>
			...
			<c:choose>
				<c:when test="${userSession.user != null}">
					<br />
					<div class="small">
						Deseja editar uma regra cadastrada por você? Vá até seu <a
							href="<c:url value='/user/profile'/>">Perfil</a>.
					</div>
					<button class="button"
						onclick="window.location.href='<c:url value='/rules/newrule'/>'">Cadastrar
						nova regra</button>
				</c:when>
				<c:otherwise>
					<br />
					<div class="small">
						<a href="<c:url value='/signup'/>">Registre-se</a> no Sistema CATA
						para poder cadastrar novas regras.
					</div>
				</c:otherwise>
			</c:choose>
		</div>
