...
<link href="<c:url value='/css/style.css'/>" rel="stylesheet"
	type="text/css" />
<link href="<c:url value='/css/jquery-ui-1.8.16.custom.css'/>"
	rel="stylesheet" type="text/css" />
<link href="<c:url value='/css/jquery-ui-multiselect-widget.css'/>"
	rel="stylesheet" type="text/css" />
<link href="<c:url value='/css/jquery.multiselect.filter.css'/>"
	rel="stylesheet" type="text/css" />
...
<script type="text/javascript"
	src="<c:url value='/js/advanced-form.js'/>"></script>
<script type="text/javascript" src="<c:url value='/js/advice-form.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/js/jquery.multiselect.min.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/js/jquery.multiselect.br.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/js/jquery.multiselect.filter.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/js/jquery.multiselect.filter.br.js'/>"></script>
<script type="text/javascript"
...
<body>
	...
	<div id="page">
		<div id="content">
			<form id="advice_form" class="width650"
				action="<c:url value="/advanced"/>" enctype="multipart/form-data"
				method="post">
				<fieldset>
					<legend>Verificação avançada</legend>
					<div class="single_form_element">
						<select id="selectCategory" name="filter"
					...
					<div id="filterForm" class="bordered-element" style="display: none">
						<div class="indentation" style="margin-top: 18px">
							<div class="single_form_element">
								<b>Escolher regras por</b>&nbsp; <select id="selectFilter"
									name="selectedFilter" class="input_border width250">
							...
							<div id="selectUserDiv" style="display: none">
								<br /> <select id="selectUser" class="width500"
									name="selectedUsers" multiple="multiple">
							...
							<div id="selectSourceDiv" style="display: none">
								<br /> <select id="selectSource" class="width500"
									name="selectedSources" multiple="multiple">
									<c:forEach var="source" items="${sources}">
										<c:choose>
											<c:when test="${source.type == 'INTERNET'}">
												<option value="${source.sourceID}"><c:out
														value="${source.title}" />,
													<c:out value="${source.url}" /></option>
											</c:when>
											<c:when test="${source.type == 'OTHER'}">
												<option value="${source.sourceID}"><c:out
														value="${source.moreInformation}" /></option>
											</c:when>
											<c:otherwise>
												<option value="${source.sourceID}"><c:out
														value="${source.title}" />,
													<c:out value="${source.authors}" /></option>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</select> <br /> <br />
					...
					<br /> Selecione um arquivo .txt, .pdf, .doc ou .tex para análise:<br>
					<div class="single_form_element">
						<input id="file" type="file" name="file" size="30"><br>
						<span class="small" style="position: relative"> <input
							id="pt" name="language" value="0" type="radio">Português
							<input id="en" name="language" value="1" type="radio">Inglês<br />
						</span>
					</div>
